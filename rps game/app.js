...
let playerSelection = (choice) => {
    ...
    if (choice === rpsArray[1]) {
        return paper; 
    }
    if (choice === rpsArray[2]) {
        return scissor; 
    }
}

// we don't need a if statement bc of early return
let playerScore = 0
let computerScore = 0
...
rockClick = () => {
    score();
    playRound(computerSelection(), playerSelection(rock));
}
paperClick = () => {
    score();
    playRound(computerSelection(), playerSelection(paper));
}
scissorClick = () => {
    score();
    playRound(computerSelection(), playerSelection(scissor));
}
